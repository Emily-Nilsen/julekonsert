...
export function ThemeSelector(props) {
  ...
  return (
    ...
    >
      ...
      >
        <LightIcon className="hidden h-4 w-4 fill-amber-500 [[data-theme=light]_&]:block" />
        <DarkIcon className="hidden h-4 w-4 fill-amber-400 [[data-theme=dark]_&]:block" />
      ...
      <Listbox.Options className="absolute left-1/2 top-full mt-3 w-36 -translate-x-1/2 space-y-1 rounded-xl bg-white p-3 text-sm font-medium shadow-md shadow-black/5 ring-1 ring-black/5 dark:bg-gray-800 dark:ring-white/5">
        {themes.map((theme) => (
          <Listbox.Option
            key={theme.value}
            value={theme}
            className={({ active, selected }) =>
              clsx(
                ...
                {
                  'text-gray-950 dark:text-amber-400': selected,
